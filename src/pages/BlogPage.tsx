            ...
            <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={onClick}
                ...
                {post.cover_image && (
                    <div style={{ width: "100%", height: 180, overflow: "hidden", flexShrink: 0 }}>
                        <img src={fixMediaUrl(post.cover_image) || ""} alt={post.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transform: hov ? "scale(1.04)" : "scale(1)", transition: "transform .4s ease" }} />
                    </div>
                )}
    ...
    return (
        <div style={{ minHeight: "100vh", background: DARK, color: "#fff", fontFamily: "'Inter','Helvetica Neue',sans-serif", position: "relative", overflowX: "hidden" }}>
            ...
            <main style={{ position: "relative", zIndex: 1, maxWidth: 1280, margin: "0 auto", padding: "clamp(100px,12vw,130px) clamp(16px,5vw,60px) 100px" }}>
                ...
                {featured && !search && (
                    <FadeIn>
                        <div onClick={() => setActiveSlug(featured.slug)} style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(34,197,94,0.18)", borderRadius: 20, overflow: "hidden", cursor: "pointer", marginBottom: 28, display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: 280 }}>
                            {featured.cover_image && <img src={fixMediaUrl(featured.cover_image) || ""} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />}
                            <div style={{ padding: "32px 28px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                                <CatBadge cat={featured.tags[0]?.name || "Featured"} />
                                <h2 style={{ margin: "14px 0", fontSize: "clamp(18px,2.5vw,26px)", fontWeight: 800, color: "#fff" }}>{featured.title}</h2>
                                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>❤️ {featured.likes_count} · 💬 {featured.comments_count}</div>
                            </div>
                        </div>
                    </FadeIn>
                )}
